...
class AddNewPeriodDataIntegrityTest extends HBaseConnectionManager with Paths with WordSpecLike with Matchers with BeforeAndAfterAll with ExistingData with DataConsistencyCheck with HFileTestUtils {
  ...
  def areAllIdsMatch(ents: Seq[Enterprise], links: Seq[LinkRecord], lous: Seq[LocalUnit]): Boolean = {
  ...
  def checkIntegrity(ents: Seq[Enterprise], links: Seq[LinkRecord], lous: Seq[LocalUnit]): Unit = {
  ...
  def isNewId(id: String): Boolean = id.startsWith("N")

  def readLinks(implicit spark: SparkSession): Seq[LinkRecord] = {
